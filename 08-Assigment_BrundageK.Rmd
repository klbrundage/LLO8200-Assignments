...
```{r Load the Carvana Data from Kaggle}
training <- read_excel("C:/Users/klbru/OneDrive/R/LLO 8200 - Intro to Data Science/08-Assignment Files/training.xlsx")
View(training)
```
...
#0: Did not get a car that is a lemon
#1: Did get a car that is a lemon
```
...
```{r Conditional Mean by Make of Car, echo=TRUE}
...
```{r Logit Prediction Table, echo=TRUE}
predlogitlm_table <- table(training$IsBadBuy,training$pred_logitlm_out)

predlogitlm_table
```

```{r Probability Logit Table Clean-up, echo=TRUE}
...
```{r Confusion Matrix from Linear Model, echo=TRUE}
